...
KIẾN TRÚC MODEL (drowsiness_asic_lite — build_asic_model):
  Input: (128, 128, 1)  Q0.7 int8
  Block 1: SeparableConv2D(32,  3×3, padding=same, use_bias=False) + BN + ReLU6 → MaxPool(2×2)
  Block 2: SeparableConv2D(64,  3×3, padding=same, use_bias=False) + BN + ReLU6 → MaxPool(2×2)
  Block 3: SeparableConv2D(128, 3×3, padding=same, use_bias=False) + BN + ReLU6 → MaxPool(2×2)
  Block 4 (Residual @ 16×16×128):
    skip = Conv2D(128, 1×1, no_bias) + BN + ReLU6  [proj shortcut]
    main = SeparableConv2D(128, 3×3, no_bias) + BN + ReLU6
    out  = Add(main, skip) → ReLU6
  GAP → Dense(64, use_bias=False) + BN + ReLU6 + Dropout(0.3)
      → Dense(1, use_bias=True)  [logit output]

  LƯU Ý: Tất cả SeparableConv2D và Conv2D proj đều dùng use_bias=False.
          FC1 (Dense 64) dùng use_bias=False; FC2 (Dense 1) mới có bias thực sự.
          BN được fold vào bias tensor trong TFLite int8;
          Dropout chỉ hoạt động lúc training, inference bỏ qua.

TENSOR MAPPING THỰC TẾ (từ debug_tensors.py — kiến trúc ASIC Lite với Residual):
  ...
  Weights accessible sau invoke:
    idx=2  → DW Block1  [1,3,3,1]      (n_scales=1)
    ...
    idx=23 → Conv4_proj [128,1,1,128]  ← skip/proj Conv2D (1×1, Block4)
    idx=26 → DW Block4  [1,3,3,128]
    idx=29 → PW Block4  [128,1,1,128]
    idx=34 → FC1 weight [64,128]
    idx=37 → FC2 weight [1,64]         (n_scales=1)

  Biases (BN fold → bias tensor mang giá trị BN; use_bias=False có bias=0):
    idx=3  → DW Block1 bias  [1]   ← zero (use_bias=False)
    idx=6  → PW Block1 bias  [32]  ← BN folded
    idx=10 → DW Block2 bias  [32]  ← zero
    idx=13 → PW Block2 bias  [64]  ← BN folded
    idx=17 → DW Block3 bias  [64]  ← zero
    idx=20 → PW Block3 bias  [128] ← BN folded
    idx=24 → conv4_proj bias [128] ← BN folded
    idx=27 → DW Block4 bias  [128] ← zero
    idx=30 → PW Block4 bias  [128] ← BN folded
    idx=35 → FC1 bias  [64]        ← BN folded (use_bias=False + BN)
    idx=38 → FC2 bias  [1]         ← có giá trị thực (use_bias=True)

  Accessible intermediate activations (XNNPACK):
    idx=33 → GAP output       [1,128]
    idx=36 → FC1+ReLU out     [1,64]
    idx=39 → Final output     [1,1]

  NOTE: Conv/Pool/Add layer outputs KHÔNG accessible → được simulate bằng numpy.
...
def write_pipeline_overview(out_dir, model_path, logit, prob, out_raw_int8,
    ...
    with open(p, 'w', encoding='utf-8') as f:
        ...
        f.write(f"       ↓  SepConv(32, 3×3, BN, ReLU6): DW + PW\n")
        f.write(f"  [Block1 out]  int8(1×128×128×32)\n")
        f.write(f"       ↓  MaxPool(2×2, s=2)\n")
        f.write(f"  [Pool1 out]   int8(1×64×64×32)\n")
        f.write(f"       ↓  SepConv(64, 3×3, BN, ReLU6): DW + PW\n")
        f.write(f"  [Block2 out]  int8(1×64×64×64)\n")
        f.write(f"       ↓  MaxPool(2×2, s=2)\n")
        f.write(f"  [Pool2 out]   int8(1×32×32×64)\n")
        f.write(f"       ↓  SepConv(128, 3×3, BN, ReLU6): DW + PW\n")
        f.write(f"  [Block3 out]  int8(1×32×32×128)\n")
        f.write(f"       ↓  MaxPool(2×2, s=2)\n")
        f.write(f"  [Pool3 out]   int8(1×16×16×128)\n")
        f.write(f"       ↓  Block4 Residual:\n")
        f.write(f"       │    skip = Conv1×1(128,BN,ReLU6)         proj4 branch\n")
        f.write(f"       │    main = SepConv(128,3×3,BN,ReLU6)     conv4 branch\n")
        f.write(f"       ↓  Add(main, skip) → ReLU6\n")
        f.write(f"  [Block4 out]  int8(1×16×16×128)\n")
        f.write(f"       ↓  GlobalAvgPool (H=16, W=16, N_spatial=256)\n")
        f.write(f"  [GAP out]     int8(128,)\n")
        f.write(f"       ↓  Dense(Cin=128, Cout=64, BN, ReLU6)  [no bias, BN folded]\n")
        f.write(f"  [FC1 out]     int8(64,)\n")
        f.write(f"       ↓  Dense(Cin=64, Cout=1)   — logit output  [bias=True]\n")
...
# ═════════════════════════════════════════════════════════════════════════════
#
# Kiến trúc: drowsiness_asic_lite — Residual Block4, FC1=64, BN trên tất cả.
# use_bias=False cho DW/PW/proj/FC1; chỉ FC2 (Dense 1) có bias thực sự.
# BN được fold vào bias tensor trong TFLite int8 (BN bias ≠ zero).
#
#  WEIGHT TENSORS (accessible):
#   idx=2  → DW  Block1  kernel [1,3,3,1]
...
#   idx=23 → Conv4_proj  kernel [128,1,1,128]  ← skip branch Conv2D 1×1
#   idx=26 → DW  Block4  kernel [1,3,3,128]
#   idx=29 → PW  Block4  kernel [128,1,1,128]
#   idx=34 → FC1 weight         [64,128]
#   idx=37 → FC2 weight         [1,64]
#
#  BIAS TENSORS (BN folded):
#   idx=3  [1]   → DW Block1 bias  ← zero
#   idx=6  [32]  → PW Block1 bias  ← BN folded
#   idx=10 [32]  → DW Block2 bias  ← zero
#   idx=13 [64]  → PW Block2 bias  ← BN folded
#   idx=17 [64]  → DW Block3 bias  ← zero
#   idx=20 [128] → PW Block3 bias  ← BN folded
#   idx=24 [128] → conv4_proj bias ← BN folded
#   idx=27 [128] → DW Block4 bias  ← zero
#   idx=30 [128] → PW Block4 bias  ← BN folded
#   idx=35 [64]  → FC1 bias        ← BN folded
#   idx=38 [1]   → FC2 bias        ← có giá trị thực (use_bias=True)
#
#  ACTIVATION TENSORS accessible (TFLite + XNNPACK):
#   idx=0  [1,128,128,1] → INPUT
#   idx=33 [1,128]       → GAP output       ← GOLDEN (exact)
#   idx=36 [1,64]        → FC1+ReLU output  ← GOLDEN (exact)
#   idx=39 [1,1]         → Final output     ← GOLDEN (exact)
#
#  NOTE: Conv, Pool, Add layer outputs đều "inaccessible" → simulate numpy.
#  Tensor indices cho intermediate activations (inaccessible, chỉ lấy scale):
#   idx=4  → DW1 out     idx=7  → PW1 out    idx=8  → Pool1 out
#   idx=11 → DW2 out     idx=14 → PW2 out    idx=15 → Pool2 out
#   idx=18 → DW3 out     idx=21 → PW3 out    idx=22 → Pool3 out
#   idx=25 → proj4 out   idx=28 → DW4 out    idx=31 → PW4 out
#   idx=32 → Add4 out (sau ReLU)
...
TENSOR_MAP = {
    ...
    'dw4_kernel':   {'idx': 26, 'layout': 'DW [1,KH,KW,128]'},
    ...
    'pw4_kernel':   {'idx': 29, 'layout': 'PW [128,1,1,128]'},
    # Proj skip kernel: [Cout, 1, 1, Cin]
    'proj4_kernel': {'idx': 23, 'layout': 'PW(proj) [128,1,1,128]'},
    # FC weights:   [Cout, Cin]
    'fc1_weight':   {'idx': 34, 'layout': 'FC [64,128]'},
    'fc2_weight':   {'idx': 37, 'layout': 'FC [1,64]'},
    # Biases: int32
    'dw1_bias':   {'idx': 3},    # [1]
    'pw1_bias':   {'idx': 6},    # [32]  BN folded
    'dw2_bias':   {'idx': 10},   # [32]
    'pw2_bias':   {'idx': 13},   # [64]  BN folded
    'dw3_bias':   {'idx': 17},   # [64]
    'pw3_bias':   {'idx': 20},   # [128] BN folded
    'proj4_bias': {'idx': 24},   # [128] BN folded
    'dw4_bias':   {'idx': 27},   # [128]
    'pw4_bias':   {'idx': 30},   # [128] BN folded
    'fc1_bias':   {'idx': 35},   # [64]  BN folded
    'fc2_bias':   {'idx': 38},   # [1]   bias thực
    # Activations accessible:
    'gap_out':    {'idx': 33},   # [1,128]  GOLDEN
    'fc1_out':    {'idx': 36},   # [1,64]   GOLDEN
    'out':        {'idx': 39},   # [1,1]    GOLDEN
}
...
def generate_all_layer_files(model_path, q07_input, out_dir):
    ...
    pw3_k    = gt('pw3_kernel')
    pw4_k    = gt('pw4_kernel')
    proj4_k  = gt('proj4_kernel')   # ← skip branch Block4
    fc1_w    = gt('fc1_weight')
    fc2_w    = gt('fc2_weight')
    ...
    dw3_b    = gt('dw3_bias')
    pw3_b    = gt('pw3_bias')
    proj4_b  = gt('proj4_bias')     # ← skip branch bias (BN folded)
    dw4_b    = gt('dw4_bias')
    pw4_b    = gt('pw4_bias')
    fc1_b    = gt('fc1_bias')
    fc2_b    = gt('fc2_bias')
    ...
    gap_out_t = gt('gap_out')   # idx=33 [1,128]
    fc1_out_t = gt('fc1_out')   # idx=36 [1,64]  (FC1+ReLU fused)
    out_t     = get_t(interp, out_det['index'])  # idx=39 [1,1]
    ...
    proj4_out_sc, proj4_out_zp = get_scale(25)  # proj skip out (Block4)
    dw4_out_sc, dw4_out_zp  = get_scale(28)   # DW Block4 out
    pw4_out_sc, pw4_out_zp  = get_scale(31)   # PW Block4+ReLU out
    add4_out_sc, add4_out_zp = get_scale(32)  # Add + ReLU4b out
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # BLOCK 4: Residual @ 16×16×128
    #   skip: proj4 Conv2D(128, 1×1) + BN + ReLU6
    #   main: DW(128) + PW(128) + BN + ReLU6
    #   out : Add(main, skip) → ReLU6
    # ─────────────────────────────────────────────────────────────────────────
    print("  📐 Simulating Block 4 (Residual)...")

    # Skip / proj branch: Conv2D 1×1  [pool3 → proj4]
    proj4_out_data = sim_pointwise_conv(pool3_info, proj4_k, proj4_b,
                                        proj4_out_sc, proj4_out_zp, has_relu=True)
    proj4_out_info = make_tensor_info('proj4_out', proj4_out_data,
                                      proj4_out_sc, proj4_out_zp,
                                      np.array([proj4_out_sc]))
    d = write_algo_pointwise_conv(out_dir, layer_num, 'conv4_proj', pool3_info,
                                   proj4_k, proj4_b, proj4_out_info, has_relu=True)
    layer_dirs.append(d); layer_num += 1
    ...
    # Add + ReLU6: simulate element-wise Add in float, requant to add4_out scale
    pw4_float  = (pw4_out_data.astype(np.float32)  - pw4_out_zp)  * pw4_out_sc
    proj4_float = (proj4_out_data.astype(np.float32) - proj4_out_zp) * proj4_out_sc
    add4_float = pw4_float + proj4_float
    # ReLU6 in float (clip to [0, 6])
    add4_float = np.clip(add4_float, 0.0, 6.0)
    add4_raw   = np.round(add4_float / add4_out_sc + add4_out_zp).clip(-128, 127).astype(np.int8)
    add4_info  = make_tensor_info('add4_out', add4_raw,
                                   add4_out_sc, add4_out_zp,
                                   np.array([add4_out_sc]))
    # Write a simple Add layer description
    add4_dir = os.path.join(out_dir, f"layer_{layer_num:02d}_skip4_Add_ReLU")
    os.makedirs(add4_dir, exist_ok=True)
    with open(os.path.join(add4_dir, "ALGO.txt"), 'w', encoding='utf-8') as f:
        f.write("=" * 72 + "\n")
        f.write(f"LAYER {layer_num:02d}: ADD + ReLU6 — skip4 (Residual)\n")
        f.write("=" * 72 + "\n\n")
        f.write("MÔ TẢ:\n")
        f.write("  Element-wise cộng hai nhánh:\n")
        f.write(f"    A = PW Block4 output  (main branch)  scale={pw4_out_sc:.10f}, zp={pw4_out_zp}\n")
        f.write(f"    B = proj4 output      (skip branch)  scale={proj4_out_sc:.10f}, zp={proj4_out_zp}\n")
        f.write(f"    C = Add(A, B) → ReLU6               scale={add4_out_sc:.10f}, zp={add4_out_zp}\n\n")
        f.write("THUẬT TOÁN (float-domain, requant sau):\n")
        f.write("  float A_real = (A_int8 - A_zp) × A_scale\n")
        f.write("  float B_real = (B_int8 - B_zp) × B_scale\n")
        f.write("  float sum    = A_real + B_real\n")
        f.write("  float relu6  = clip(sum, 0.0, 6.0)\n")
        f.write("  int8  out    = clip(round(relu6 / C_scale + C_zp), -128, 127)\n\n")
        f.write("NOTE: TFLite Add op tự rescale hai nhánh về cùng domain trước khi cộng.\n")
        f.write("      RTL cần 2 rescale multiplier (một cho A, một cho B) rồi cộng int.\n")
    hdr_a   = f"// Add input A (pw4): shape={list(pw4_out_data.shape)}"
    hdr_b   = f"// Add input B (proj4): shape={list(proj4_out_data.shape)}"
    hdr_out = f"// Add+ReLU6 output: shape={list(add4_raw.shape)}, sc={add4_out_sc:.8f}, zp={add4_out_zp}"
    write_hex_file(os.path.join(add4_dir, "input_A_pw4.hex.txt"),   pw4_out_data,  hdr_a)
    write_int_file(os.path.join(add4_dir, "input_A_pw4.int.txt"),   pw4_out_data,  hdr_a)
    write_hex_file(os.path.join(add4_dir, "input_B_proj4.hex.txt"), proj4_out_data, hdr_b)
    write_int_file(os.path.join(add4_dir, "input_B_proj4.int.txt"), proj4_out_data, hdr_b)
    write_hex_file(os.path.join(add4_dir, "output.hex.txt"),        add4_raw,      hdr_out)
    write_int_file(os.path.join(add4_dir, "output.int.txt"),        add4_raw,      hdr_out)
    write_float_file(os.path.join(add4_dir, "output.float.txt"),    add4_raw, add4_out_sc, add4_out_zp, hdr_out)
    params_add = {
        "layer_type": "Add+ReLU6",
        "input_A": {"name": "pw4_out",   "scale": pw4_out_sc,   "zero_point": pw4_out_zp},
        "input_B": {"name": "proj4_out", "scale": proj4_out_sc, "zero_point": proj4_out_zp},
        "output":  {"shape": list(add4_raw.shape), "scale": add4_out_sc, "zero_point": add4_out_zp},
    }
    with open(os.path.join(add4_dir, "params.json"), 'w') as fp:
        json.dump(params_add, fp, indent=2)
    print(f"  [{layer_num:02d}] Add+ReLU6 skip4                            out={list(add4_raw.shape)}")
    layer_dirs.append(add4_dir); layer_num += 1

    # ─────────────────────────────────────────────────────────────────────────
    # GAP — output ACCESSIBLE (idx=33), dùng làm GOLDEN
    # ─────────────────────────────────────────────────────────────────────────
    print("  📐 GAP (GOLDEN from TFLite)...")
    # Input = Add+ReLU6 output (Block4 residual)
    d = write_algo_gap(out_dir, layer_num, 'gap', add4_info, gap_out_t)
    layer_dirs.append(d); layer_num += 1

    # ─────────────────────────────────────────────────────────────────────────
    # FC1 + BN + ReLU6 — output ACCESSIBLE (idx=36), dùng làm GOLDEN
    # Dense(64, use_bias=False) + BatchNorm + ReLU6 + Dropout(0.3, inference=no-op)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # FC2 (output logit) — output ACCESSIBLE (idx=39), dùng làm GOLDEN
    # Dense(1, use_bias=True), Cin=64
    # ─────────────────────────────────────────────────────────────────────────
